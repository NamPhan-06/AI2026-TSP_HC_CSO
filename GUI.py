...
class TSPGuiApp:
    ...
    def _setup_control_panel(self):
        ...
        ttk.Label(self.control_frame, text="KẾT QUẢ CHÍNH:", font=("Arial", 12, "bold")).grid(row=6, column=0,
                                                                                              columnspan=2, pady=(5, 2),
                                                                                              sticky=tk.W)
        self.txt_main = tk.Text(self.control_frame, font=("Courier", 10), height=11, width=42, wrap=tk.WORD)
        self.txt_main.grid(row=7, column=0, columnspan=2, sticky="nsew")

        # --- BOX 2: LỊCH SỬ HỘI TỤ ---
        ttk.Label(self.control_frame, text="LỊCH SỬ TỪNG VÒNG LẶP:", font=("Arial", 12, "bold")).grid(row=8, column=0,
                                                                                                      columnspan=2,
                                                                                                      pady=(10, 2),
                                                                                                      sticky=tk.W)
        self.txt_history = tk.Text(self.control_frame, font=("Courier", 10), height=11, width=42, wrap=tk.WORD)
        self.txt_history.grid(row=9, column=0, columnspan=2, sticky="nsew")
